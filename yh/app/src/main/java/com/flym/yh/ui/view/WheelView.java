...
/**
 * Created by Morphine on 2016/11/2.
 */
...
public class WheelView extends View {
    ...
    private float textSize = 12;          //文字的大小
    private boolean isCircle = true;     //是否为环形
    private int rate = 10;               //惯性滑动比率，rate越大，速率越快
    private int textColor = 0xFF000000;   //文字颜色
    private int lineColor = 0xFFBEBEBE;   //线条的颜色
    ...
    private void init() {
        ...
        paint.setTextSize(DensityUtil.dp2px(getContext(), textSize));
    ...
    public void setProvinecData(List<? extends WheelViewData> list) {
        ...
        if (list != null && list.size() > 0)
            for (int i = 0; i < list.size(); i++) {
                addData(list.get(i).getName(), list.get(i));
            }
    }

    public void setCityData(List<? extends WheelViewData> list) {
        if (data != null && data.size() > 0) {
            data.clear();
        }
        dataSize = 0;
    ...
    public interface OnScrollListener {
        void onScrollStop(Object centerItem);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = startItemPos, j = 0; i < startItemPos + showSize + 2; j++, i++) {
            ...
            if (i >= 0 && i < dataSize) {
                String text = data.get(i).showStr;
                if (data.get(i).showStr.length() > 10) {
                    text = data.get(i).showStr.substring(0, 9) + "...";
                }
                canvas.drawText(text, itemX,
...
//                }
                float x = (-scrollY / itemHeight + (showSize - 1) / 2);
                if (scrollY < minScrollY) {
                    Log.i("xxxxxxxxxx", "min");
                    scrollY = minScrollY;
                    nowChecked = dataSize - 1;
                } else if (scrollY >= maxScrollY) {
                    Log.i("xxxxxxxxxx", "max");
                    scrollY = maxScrollY;
                    nowChecked = 0;
                } else {
                    nowChecked = Math.round(x);
                }
            }
...
//            mScroller.springBack(0,(int)scrollY,0,0,minScrollY,maxScrollY);
        ...
        scrollListener.onScrollStop(getCenterItem());
    }
