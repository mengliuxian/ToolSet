...
public class SetTelephoneConsultationAdapter extends BaseQuickAdapter<GetServicesBean.ListBean, BaseViewHolder> {

    private List<GetServicesBean.ListBean> dataList;

    public SetTelephoneConsultationAdapter(@Nullable List<GetServicesBean.ListBean> data, List<GetPhoneLongTimeBean.ListBean> listLongTime) {
        super(R.layout.item_set_telephone_consultation, data);
        dataList = data;
    }
    ...
    protected void convert(BaseViewHolder helper, final GetServicesBean.ListBean item) {
        ...
        helper.setText(R.id.tv_name, !TextUtils.isEmpty(item.getContent())?item.getContent():"请选择时间")
