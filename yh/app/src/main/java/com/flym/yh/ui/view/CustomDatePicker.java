...
public class CustomDatePicker {
    ...
    private DatePickerView year_pv, month_pv, day_pv, hour_pv, minute_pv, seconds_pv;
    ...
    private ArrayList<String> year, month, day, hour, minute, seconds;
    private int startYear, startMonth, startDay, startHour, startMinute, startSeconds, endYear, endMonth, endDay, endHour, endMinute, endSeconds;
    private boolean spanYear, spanMon, spanDay, spanHour, spanMin, spanSeconds;
    private Calendar selectedCalender, startCalendar, endCalendar;
    private TextView tv_cancle, tv_select, hour_text, minute_text, seconds_text;
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler, String startDate, String endDate) {
        if (isValidDate(startDate, "yyyy-MM-dd HH:mm:ss") && isValidDate(endDate, "yyyy-MM-dd HH:mm:ss")) {
            ...
            try {
                ...
                endCalendar.setTime(gc.getTime());
    ...
    private void initView() {
        ...
        seconds_pv = (DatePickerView) datePickerDialog.findViewById(R.id.seconds_pv);
        ...
        seconds_text = (TextView) datePickerDialog.findViewById(R.id.seconds_text);
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (seconds_pv.getVisibility() == View.VISIBLE) {
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    handler.handle(sdf.format(selectedCalender.getTime()));
                } else if (year_pv.getVisibility() == View.GONE) {
                    SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
                    handler.handle(sdf.format(selectedCalender.getTime()));
                } else if (hour_pv.getVisibility() == View.GONE) {
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                    handler.handle(sdf.format(selectedCalender.getTime()));
                } else if (hour_pv.getVisibility() == View.VISIBLE) {
    ...
    private void initParameter() {
        startYear = startCalendar.get(YEAR);
        startMonth = startCalendar.get(Calendar.MONTH) + 1;
        startDay = startCalendar.get(Calendar.DAY_OF_MONTH);
        startHour = startCalendar.get(Calendar.HOUR_OF_DAY);
        startMinute = startCalendar.get(Calendar.MINUTE);
        startSeconds = startCalendar.get(Calendar.SECOND);

        endYear = endCalendar.get(YEAR);
        endMonth = endCalendar.get(Calendar.MONTH) + 1;
        endDay = endCalendar.get(Calendar.DAY_OF_MONTH);
        endHour = endCalendar.get(Calendar.HOUR_OF_DAY);
        endMinute = endCalendar.get(Calendar.MINUTE);
        endSeconds = endCalendar.get(Calendar.SECOND);
        ...
        spanSeconds = (!spanMin) && (startSeconds != endSeconds);
    ...
    private void initTimer() {
        ...
        if (spanYear) {
            ...
            for (int i = startMonth; i >= MIN_MONTH; i--) {
...
//            }
            ...
            if ((scrollUnits & SCROLL_TYPE.SECOND.value) != SCROLL_TYPE.SECOND.value) {
                seconds.add(formatTimeUnit(startSeconds));
            } else {
                for (int i = startSeconds; i <= MAX_SECONDS; i++) {
                    seconds.add(formatTimeUnit(i));
                }
            }
        } else if (spanMon) {
            ...
            if ((scrollUnits & SCROLL_TYPE.MINUTE.value) != SCROLL_TYPE.MINUTE.value) {
                ...
                for (int i = startSeconds; i <= startSeconds; i++) {
                    seconds.add(formatTimeUnit(i));
                }
            }
        }
    ...
    private void initArrayList() {
        ...
        if (seconds == null) seconds = new ArrayList<>();
        year.clear();
        month.clear();
        day.clear();
        hour.clear();
    ...
    private void addListener() {
        year_pv.setOnSelectListener(new DatePickerView.onSelectListener() {
            ...
            public void onSelect(String text) {
                selectedCalender.set(YEAR, Integer.parseInt(text));
                monthChange();
            }
        });
        ...
        hour_pv.setOnSelectListener(new DatePickerView.onSelectListener() {
            ...
            public void onSelect(String text) {
                ...
                if (year_pv.getVisibility() == View.GONE &&
                        month_pv.getVisibility() == View.GONE &&
                        day_pv.getVisibility() == View.GONE) {
                    changeMinuteNoYear();
                } else {
                    minuteChange();
                }
        ...
        seconds_pv.setOnSelectListener(new DatePickerView.onSelectListener() {
            @Override
            public void onSelect(String text) {
    ...
    private void monthChange() {
        month.clear();
        int selectedYear = selectedCalender.get(YEAR);
    ...
    private void secondChange() {
        if ((scrollUnits & SCROLL_TYPE.MINUTE.value) == SCROLL_TYPE.MINUTE.value) {
            ...
            if (selectedYear == startYear && selectedMonth == startMonth && selectedDay == startDay && selectedHour == startHour && selectedMinute == startMinute) {
                ...
                for (int i = MIN_SECONDS; i <= endSeconds; i++) {
                    seconds.add(formatTimeUnit(i));
                }
            } else {
            ...
            selectedCalender.set(Calendar.SECOND, Integer.parseInt(seconds.get(0)));
            seconds_pv.setData(seconds);
            seconds_pv.setSelected(0);
            executeAnimator(seconds_pv);
        }
        executeScroll();
    }
...
//        ObjectAnimator.ofPropertyValuesHolder(view, pvhX, pvhY, pvhZ).setDuration(200).start();
    ...
    private void executeScroll() {
        ...
        seconds_pv.setCanScroll(seconds.size() > 1 && (scrollUnits & SCROLL_TYPE.SECOND.value) == SCROLL_TYPE.SECOND.value);
...
//                    setSelectedTime(time);
    ...
    public void changeMinuteNoYear() {
        month.clear();
        for (int i = 0; i <= 59; i++) {
            minute.add(formatTimeUnit(i));
        }
     ...
     * 设置日期控件是否显示秒
     */
    public void showSecondTime(boolean show) {
        if (canAccess) {
            if (show) {
                disScrollUnit();
                seconds_pv.setVisibility(View.VISIBLE);
                seconds_text.setVisibility(View.VISIBLE);
            } else {
                disScrollUnit(SCROLL_TYPE.SECOND);
                seconds_pv.setVisibility(View.GONE);
                seconds_text.setVisibility(View.GONE);
            }
        }
    }
    ...
    public void setIsLoop(boolean isLoop) {
        if (canAccess) {
            ...
            this.seconds_pv.setIsLoop(isLoop);
        }
    }
    ...
    public void setSelectedTime(String time) {
        if (canAccess) {
            ...
            selectedCalender.set(YEAR, Integer.parseInt(dateStr[0]));
            ...
            if (str.length == 2) {
                ...
                if ((scrollUnits & SCROLL_TYPE.SECOND.value) == SCROLL_TYPE.SECOND.value) {
                    ...
                    seconds_pv.setData(seconds);
                    seconds_pv.setSelected(timeStr[2]);
